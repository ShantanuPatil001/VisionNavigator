...
# Annoucing a process with exception
def process(text1):
...
def wishMe(namecall):
    ...
    if hour >= 0 and hour < 12 :
        process(f"Hello,Good Morning {name1}")
    elif hour>=12 and hour<18:
        process(f"Hello,Good Afternoon {name1}")
    else:
        process(f"Hello,Good Evening {name1}")
...
#
#         except Exception as e:
#             process("Pardon me, please say that again")
#             return "None"
#         return statement
...
def object_detect():
    ...
    process('objects in front of you are')
    process(s)
    time.sleep(3)
    if s!='':
        cap.release()  # video close
